...
function Dashboard() {
  ...
  const filtered = useMemo(() => {
    ...
    else if (filter === "urgent") list = tasks.filter((t) => t.priority.includes("Urgent"));
  ...
  return (
    <main className="max-w-[480px] mx-auto px-3.5 pb-24">
      ...
      {tab === "tasks" && (
        <>
          ...
          {tasksQ.isLoading ? <Loading /> :
           ...
           filtered.length === 0 ? <Empty msg={filter === "mine" ? "No tienes tareas asignadas" : "Sin tareas en este filtro"} /> :
           filtered.map((t) => (
             <TaskCard key={t.id} task={t} projects={projects} members={members} myUserId={myUserId} onOpenDetail={setDetail} />
           ))}
        </>
      )}
        ...
        <>
          {projectsQ.isLoading ? <Loading /> :
           [...projects].sort((a: any, b: any) => {
             ...
             if (a.status !== "Active" && b.status === "Active") return 1;
             return tasks.filter((t) => t.projectIds.includes(b.id)).length - tasks.filter((t) => t.projectIds.includes(a.id)).length;
      ...
      {/* WEEK TAB */}
      {tab === "week" && (
        <WeekView tasks={tasks} projects={projects} members={members} myUserId={myUserId} onOpenDetail={setDetail} />
      )}
      ...
      {detail && (
        <TaskDetail task={detail} projects={projects} members={members} onClose={() => setDetail(null)} />
      )}
    </main>
  );
}
    ...
    <>
      {overdue.length > 0 && (
        <>
          ...
          {overdue.map((t) => <TaskCard key={t.id} task={t} projects={projects} members={members} myUserId={myUserId} onOpenDetail={onOpenDetail} />)}
        </>
      )}
      {week.length > 0 && (
        <>
          ...
          {[...week].sort((a, b) => (a.dueDate || "").localeCompare(b.dueDate || ""))
            .map((t) => <TaskCard key={t.id} task={t} projects={projects} members={members} myUserId={myUserId} onOpenDetail={onOpenDetail} />)}
        </>
      )}
      {urgent.length > 0 && (
        <>
          ...
          {urgent.map((t) => <TaskCard key={t.id} task={t} projects={projects} members={members} myUserId={myUserId} onOpenDetail={onOpenDetail} />)}
        </>
      )}
    </>
  );
