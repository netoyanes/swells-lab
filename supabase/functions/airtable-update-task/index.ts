// supabase/functions/airtable-update-task/index.ts
...
// Maps client-side keys -> Airtable field names
...
serve(async (req) => {
  ...
  try {
    ...
    if (!taskId || !fields) {
      return new Response(
        JSON.stringify({ error: "Missing taskId or fields" }),
    ...
    const r = await fetch(
      `https://api.airtable.com/v0/${baseId}/${tableId}/${taskId}`,
      {
        method: "PATCH",
        headers: {
